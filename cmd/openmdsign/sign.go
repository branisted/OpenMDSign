...
func newSignCmd(gf *globalFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "sign",
		Short: "Sign a document with the token into an AdES container (PAdES for PDF)",
		Long: "sign produces a real AdES signature with the hardware token.\n\n" +
			...
			"signature timestamp from the TSA. XAdES (non-PDF) arrives in Phase 2b.\n\n" +
			"Certificate chain: the token holds only the leaf. Pass --chain <file> (a PEM\n" +
			"bundle of issuing CA + root) to embed the full chain in the CMS. The public\n" +
			"MoldSign chain can be fetched from:\n" +
	...
	cmd.Flags().StringVar(&f.out, "out", "", "output path (default ./sign-out/<basename>.signed.pdf)")
	cmd.Flags().StringVar(&f.profile, "profile", "auto", "signature profile: auto | pades")
	...
	cmd.Flags().StringVar(&f.chain, "chain", "", "PEM bundle of the issuer chain (issuing CA + root) to embed")
...
func runSign(cmd *cobra.Command, gf *globalFlags, f *signFlags) error {
	...
	// Load the issuer chain, if provided. Absence is a warning, not an error.
	...
	if outPath == "" {
		...
		outPath = filepath.Join("sign-out", base+".signed.pdf")
	}
	...
	// Build the profile Signer. Only PAdES ships today.
...
func resolveProfile(profile, file string) (string, error) {
	...
	case "auto", "":
		...
		return "", fmt.Errorf("no signer for %q inputs yet: XAdES is not yet implemented (Phase 2b). "+
			"Only PDF (PAdES) is supported today", ext)
	default:
		return "", fmt.Errorf("invalid --profile %q: expected \"auto\" or \"pades\"", profile)
	}
}
...
func printSignText(signer *token.Signer, res sign.Result, chainPath string, chainLen int, tsaURL string) {
	...
	fmt.Fprintf(w, "  Signed PDF:      %s (%d bytes)\n", res.OutputPath, res.Bytes)
}
