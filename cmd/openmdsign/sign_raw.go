...
// buildSHA256DigestInfo returns the DER-encoded DigestInfo for a SHA-256 digest.
func buildSHA256DigestInfo(digest []byte) []byte {
...
func runSignRaw(cmd *cobra.Command, gf *globalFlags, f *signRawFlags) error {
	...
	if mech.onTokenHash {
		...
		payload = buildSHA256DigestInfo(digest[:]) // token raw-signs the DigestInfo
	}
